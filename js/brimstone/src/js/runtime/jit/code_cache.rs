...
use super::abi::{ActivationOwner, GeneratedEntry};
...
impl ExecutableMemory {
    ...
    pub(crate) fn mapped_len_for(code_len: usize) -> Result<usize, CodeMemoryError> {
        if code_len == 0 {
    ...
    pub(crate) unsafe fn call(
        &self,
        activation: &mut ActivationOwner<'_, '_>,
...
impl ExecutableCodeCache {
    ...
    pub(crate) fn insert(&mut self, key: u64, bytes: &[u8]) -> Result<(), CodeMemoryError> {
        self.insert_observed(key, bytes, |_, _| {})
    }
    ...
    fn insert_observed(
        ...
        self.ensure_owner()?;
        let mapped_len = ExecutableMemory::mapped_len_for(bytes.len())?;
        ...
        if let Some(index) = self.entries.iter().position(|entry| entry.key == key) {
            ...
            self.used_bytes = self
                .used_bytes
                .checked_sub(old.code.mapped_len())
        ...
        let code = ExecutableMemory::from_bytes_observed(bytes, observe)?;

        self.clock = self.clock.wrapping_add(1);
        self.used_bytes = self
            .used_bytes
            .checked_add(code.mapped_len())
            .ok_or(CodeMemoryError::SizeOverflow)?;
        self.entries
            .push_back(CacheEntry { key, last_used: self.clock, code });
        Ok(())
    }

    pub(crate) fn get(&mut self, key: u64) -> Result<Option<&ExecutableMemory>, CodeMemoryError> {
        self.ensure_owner()?;
        ...
        Ok(Some(&self.entries[index].code))
    }
    ...
    fn evict_lru(&mut self) {
        ...
        self.used_bytes -= entry.code.mapped_len();
    }
...
mod tests {
    ...
    fn cache_enforces_capacity_lru_eviction_and_mapping_accounting() {
        ...
        assert_eq!(test_live_mapping_bytes(), 0);
        {
            ...
            cache.insert(1, &[0xc3]).unwrap();
            cache.insert(2, &[0xc3]).unwrap();
            let first_address = cache.get(1).unwrap().unwrap().start_address();
    ...
    fn limits_reject_empty_oversized_and_zero_capacity_inputs() {
        ...
        let oversized = vec![0xc3; page + 1];
        assert!(matches!(cache.insert(1, &oversized), Err(CodeMemoryError::CodeTooLarge { .. })));
