...
fn assert_pre_submission_rejections(
    ...
    let revision = output.document_revision;
    let stale = renderer
        .render(compile(output), FrameRequest::new(revision + 1, 1))
        .unwrap_err();
    assert!(matches!(
        stale,
        HeadlessError::StaleRevision {
            expected,
            actual
    ...
    let mismatch = renderer
        .render(
            ...
            FrameRequest::new(wrong_viewport.document_revision, 1),
        )
        .unwrap_err();
    assert!(matches!(
        mismatch,
...
fn assert_decoration_frames(
    ...
    let revision = output.document_revision;
    let first = renderer
        .render(compile(output), FrameRequest::new(revision, 1))
    ...
    assert_eq!(first.document_revision(), revision);
...
fn assert_resource_rejection(size: FrameSize, output: &wild_buzzard_layout::LayoutOutput) {
    ...
    let resource_error = renderer
        .render(
            compile(output),
            FrameRequest::new(output.document_revision, 1),
        )
        .unwrap_err();
    assert!(matches!(
        resource_error,
...
fn two_live_renderers_restore_their_own_contexts() {
    ...
    let second_again = second
        .render(
            compile(&output),
            FrameRequest::new(output.document_revision, 2),
        )
