...
impl ScaleOffset {
    ...
    pub fn is_identity(&self) -> bool {
        self.scale.x == 1.0 && self.scale.y == 1.0 && self.offset.x == 0.0 && self.offset.y == 0.0
    }
...
impl<Src, Dst> MatrixHelpers<Src, Dst> for Transform3D<f32, Src, Dst> {
    ...
    fn as_grid_aligned_rotation(&self) -> Option<(ScaleOffset, bool)> {
        ...
        if is_unit(self.m11) && is_unit(self.m22) && is_zero(self.m12) && is_zero(self.m21) {
            Some((
                ScaleOffset::new(self.m11, self.m22, self.m41, self.m42),
                false,
            ))
        } else if is_unit(self.m12) && is_unit(self.m21) && is_zero(self.m11) && is_zero(self.m22) {
            Some((
                ScaleOffset::new(self.m21, self.m12, self.m41, self.m42),
                true,
            ))
        } else {
            None
...
pub mod test {
    ...
    fn map_unmap_negative_size() {
        ...
        assert_eq!(
            mapped_rect,
            Box2D::new(Point2D::new(11.0, 11.0), Point2D::new(11.0, 11.0))
        );

        let unmapped_rect: Box2D<f32> = scale_offset.unmap_rect(&rect);
        assert_eq!(
            unmapped_rect,
            Box2D::new(Point2D::new(2.0, 2.0), Point2D::new(2.0, 2.0))
        );
    }
